...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    char *name_list = option_find_str(options, "names", "data/names.list"); //判断options里面是否含有names信息，如果没有则默认为data/names.list
    char **names = get_labels(name_list); //获取类别名称,eg: person car

    image **alphabet = load_alphabet(); //使最后检测结果图片上能显示文字信息
    network *net = load_network(cfgfile, weightfile, 0); //读取网络结构，并分配权重，net包含了整个网络的信息
    set_batch_network(net, 1); //将batch数目强制置为1，因为一次只能检测一张图片
    srand(2222222);
    double time;
    char buff[256];
    char *input = buff;
    float nms=.45; //非极大值抑制参数??
    while(1){
        if(filename){
        ...
        image sized = letterbox_image(im, net->w, net->h); //调整为网络输入大小
        ...
        float *X = sized.data; //喂个网络的数据
        time=what_time_is_it_now(); 
        network_predict(net, X); //网络前馈，这里并没有直接run出最后的结果，还需要对网络给出来的结果进行解析
        printf("%s: Predicted in %f seconds.\n", input, what_time_is_it_now()-time);
        int nboxes = 0;
        detection *dets = get_network_boxes(net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes); //这里就是完成前面网络结果的解析
        //printf("%d\n", nboxes);
        //if (nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);  //非极大值j抑制
