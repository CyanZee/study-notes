...
{  //*net是网络的结果，w h 分别是原始图片的宽高， thresh是置信度阈值， 大于这个阈值就认为这个候选框是有物体的，*num是候选框的数目
    detection *dets = make_network_boxes(net, thresh, num); //主要是通过阈值来选出一定数目的候选框，初筛
    fill_network_boxes(net, w, h, thresh, hier, map, relative, dets); //选出来的候选框的坐标都是相对位置，这里对其进行在原始图片的配对
    return dets;
}
